...
2a. A PHOTOGRAPH THAT FADES OUT HAS NO EDGE, AND NO EDGE MEANS OVERLAY. This is the single most expensive misreading there is, so apply the test literally: could you draw a straight line down the poster where the photograph stops? A hard boundary — a crop, a frame, a change of colour along a clean vertical — is a column. A photograph that blends, blurs, blows out to white or dissolves into the background on one side has no such line, and the pale area is part of the photograph, not a space beside it. The copy sitting on that pale area is ON the photograph. Emit ONE full-width cell holding the photo and that copy together.

Judge it by the photograph, never by the copy. A clinic poster whose stethoscope occupies the right and washes to near-white on the left, with a service list on the washed side, reads at a glance as "list on the left, photo on the right" and is in fact one full-width photograph with a list over it. Splitting it produces a poster that is half white column, which is the commonest way a rebuild comes out emptier than the reference it was read from.
