...
- ctaLabel: the words inside the poster's button. Two to four words, an instruction rather than a description: "BOOK A SITE VISIT", "GET YOUR FREE QUOTE", "START INVESTING NOW". **At most 28 characters, counted including spaces** — it is set inside a button, not across a bar, and one over the limit is discarded rather than shortened, so count before you answer. Make it follow from the day's subject, so a campaign's ask varies across the month instead of repeating one line for a year.
