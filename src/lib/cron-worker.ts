...
import {
  runQueuedCampaignGenerations,
  type GenerationSweepResult,
 ...
 *
 * **Phases 4 and 5 are campaign work**, and neither touches a legacy row.
 * Phase 4 drains the server-side poster generation queue
 * (`ContentCalendar.generationStatus = QUEUED`, campaign days only) added in
 * Phase 7; phase 5 sends due campaign deliveries from `CampaignDelivery` rows
 * (Phase 6). Both are wrapped so a failure in them cannot lose the legacy work
 * already done above.
 */
...
export interface DispatchSummary {
  ...
  /** Campaign posters generated by the server-side queue this sweep (Phase 7). */
...
export async function executeIntervalDispatch(
  ...
  // ---- Phase 4: queued campaign poster generation (Phase 7 feature) --------
  //
  // Campaign generation used to run only in the browser, one request per day,
  // so closing the tab stopped a batch. An operator now queues the work and
  // this drains it: `ContentCalendar.generationStatus = QUEUED` is the queue,
  // and every claim is the same conditional update Phase 4 used.
  //
  // Bounded twice (days per tick, and concurrency within a tick) because each
  // day is an image model call plus a composite plus two Drive writes, and this
  // shares a request budget with the legacy phases above.
  let campaignGeneration: GenerationSweepResult = { claimed: 0, generated: [], failed: [], skipped: [], stopped: false };
  try {
    campaignGeneration = await runQueuedCampaignGenerations(prisma, { now });
  } catch (error) {
    console.error('[ace:cron] campaign generation sweep failed', error instanceof Error ? error.message : error);
  }

  // ---- Phase 5: campaign deliveries that are due (Phase 6 feature) ---------
  //
